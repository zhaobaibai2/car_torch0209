...
class CarPVPTD3TorchEval(Node):
    def __init__(self, *, noisy: bool):
        super().__init__("car_pvp_td3_torch_eval")
    ...
    def _load_config(self) -> dict:
        ...
        save_folder = os.path.join(
            base_save,
            "results",
            "PVP_TD3_EVAL_" + datetime.datetime.now().strftime("%y%m%d-%H%M%S"),
        )
    ...
    def _compute_action(self, state: np.ndarray) -> Tuple[np.ndarray, np.ndarray, float]:
        t0 = time.time()
        action = self.model.predict(state, deterministic=self.eval_deterministic)
...
def _main(noisy: bool):
    rclpy.init()
    node = CarPVPTD3TorchEval(noisy=noisy)
    try:
        rclpy.spin(node)
    finally:
        node.destroy_node()
