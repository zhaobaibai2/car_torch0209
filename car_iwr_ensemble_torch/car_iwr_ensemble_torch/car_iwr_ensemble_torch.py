...
class CarIWREnsembleTorch(Node):
    def __init__(self):
        ...
        self.timerCar = self.create_timer(0.1, self.pub_callback_car_iwr_ensemble_torch)
        ...
        self.dataset_X = deque()
        self.dataset_y = deque()
    ...
    def _load_config(self) -> dict:
        ...
        save_folder = os.path.join(
            base_save,
            "results",
            "IWR_CAR_" + datetime.datetime.now().strftime("%y%m%d-%H%M%S"),
        )
    ...
    def _init_model(self):
        ...
        if model_path:
            ...
            self.get_logger().info("Initialized new IWR Ensemble model (no checkpoint found)")

    def _init_optimizers(self):
        tr_cfg = self.config.get('training', {}) or {}
        lr = float(tr_cfg.get('lr', 1e-3))
        lambda_l2 = float(tr_cfg.get('lambda_l2', 1e-5))
        self._criterion = torch.nn.MSELoss()
        self._optimizers = [
            torch.optim.SGD(self.model.pis[i].parameters(), lr=lr, weight_decay=lambda_l2)
    ...
    def _append_dataset(self, state: np.ndarray, action_behavior: np.ndarray, intervention: float):
        if intervention <= 0.5:
            return

        tr_cfg = self.config.get('training', {}) or {}
        max_size = int(tr_cfg.get('max_dataset_size', 50000))

        self.dataset_X.append(np.asarray(state, dtype=np.float32))
        self.dataset_y.append(np.asarray(action_behavior, dtype=np.float32))
        while len(self.dataset_X) > max_size:
            self.dataset_X.popleft()
            self.dataset_y.popleft()
    ...
    def _train_step(self):
        ...
        bs = int(tr_cfg.get('batch_size', 256))
        epochs = int(tr_cfg.get('epochs_per_update', 1))
        if len(self.dataset_X) < bs:
            return

        X = np.stack(list(self.dataset_X), axis=0)
        y = np.stack(list(self.dataset_y), axis=0)

        for _ in range(max(1, epochs)):
            idx = np.random.randint(0, X.shape[0], size=bs)
            xb = torch.as_tensor(X[idx], dtype=torch.float32, device=self.device)
            yb = torch.as_tensor(y[idx], dtype=torch.float32, device=self.device)

            for net_id in range(self.model.num_nets):
                pred = self.model.pis[net_id](xb)
                loss = self._criterion(pred, yb)
                self._optimizers[net_id].zero_grad()
                loss.backward()
                self._optimizers[net_id].step()

    def _save_checkpoint(self):
        try:
            model_path = os.path.join(self.config['save_folder'], 'models', f"iwr_ensemble_{self.iteration:08d}.pth")
            self.model.save(model_path)
        except Exception as e:
            self.get_logger().error(f"Model save failed: {e}")

        try:
            data_path = os.path.join(self.config['save_folder'], 'buffers', f"dataset_{self.iteration:08d}.npz")
            if len(self.dataset_X) > 0:
                X = np.stack(list(self.dataset_X), axis=0)
                y = np.stack(list(self.dataset_y), axis=0)
                np.savez_compressed(data_path, X=X, y=y)
        except Exception as e:
            self.get_logger().error(f"Dataset save failed: {e}")
    ...
    def pub_callback_car_iwr_ensemble_torch(self):
        msg = CarRLInterface()
        now_ts = time.time()
        ...
        action_model, infer_time = self._compute_action(state)
        self.last_infer_time = float(infer_time)
        self.last_action_model = np.asarray(action_model, dtype=np.float32).copy()

        self.send_action(action_model)
        ...
        self._append_dataset(state, action_behavior, intervention)

        tr_cfg = self.config.get('training', {}) or {}
        if self.iteration % int(tr_cfg.get('update_interval', 10)) == 0:
            self._train_step()
        ...
        if tjitools is not None:
            tjitools.ros_log(self.get_name(), 'Publish car_iwr_ensemble_torch msg !!!')
