# car_iwr_ensemble_torch（IWR-Ensemble 实车节点）

本包用于将 **IWR Ensemble** 算法迁移到实车节点上运行，接口、CAN 编码、安全策略、日志指标与 `car_dacer_torch` 对齐，便于对比实验。算法代码已内置在包内（无需依赖外部 Metadrive_baseline）。

## 1. 节点与话题

- 订阅：`surrounding_info_data`（`car_interfaces/msg/SurroundingInfoInterface`）
- 发布：`car_iwr_ensemble_data`（`car_interfaces/msg/CarRLInterface`）
- CAN：socketcan (`can1` / `can0`)，编码与 `car_dacer_torch` 一致

## 2. 目录结构

```
car_iwr_ensemble_torch/
├── car_iwr_ensemble_torch/
│   ├── car_iwr_ensemble_torch.py        # 实车主节点
│   ├── car_iwr_ensemble_torch_eval.py   # 评估节点（det/noisy）
...
```bash
colcon build --packages-select car_iwr_ensemble_torch
source install/setup.bash

# 实车主节点
ros2 run car_iwr_ensemble_torch car_iwr_ensemble_torch

# 评估节点（det/noisy）
ros2 run car_iwr_ensemble_torch car_iwr_ensemble_torch_eval_det
ros2 run car_iwr_ensemble_torch car_iwr_ensemble_torch_eval_noisy
```
...
- `model.hidden_sizes / model.num_nets`：Ensemble 网络结构
- `training.*`：在线 IWR 更新开关与频率
- `eval.model_dir / eval.model_path`：加载 `.pth/.pt` 模型
...
保存目录：`/home/nvidia/Autodrive/results/IWR_CAR_时间戳/`
...
2) **模型加载失败**
- 确认 `eval.model_path` 或 `eval.model_dir` 存在 `.pth/.pt` 模型文件
